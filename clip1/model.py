...
class visTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for r in self.resblocks:
            x = r(x)
            fea.append(x[0,:,:])
        fea = torch.stack(fea).permute(1, 0, 2)
        return x, fea
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.transformer = visTransformer(width, layers, heads, prompts_needed=self.prompt_till_layer_visual,
    ...
    def forward(self, x: torch.Tensor):
        ...
        x, fea = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD

        x = self.ln_post(x[:, 0, :])
        # local = self.ln_post(x).mean(1)
